...
import {
    ...
    persistXiangqiState,
    type XiangqiHeaderInput,
} from "@/xiangqi/persistence";
import { INITIAL_XIANGQI_FEN } from "@/xiangqi/xiangqi";
...
export async function createTab({
    ...
    if (pgn !== undefined) {
        let xiangqiState;
        try {
            xiangqiState = createXiangqiStateFromNotation(pgn, headersToXiangqi(headers));
        } catch {
            try {
                xiangqiState = createXiangqiStateFromFen(
                    headers?.fen || INITIAL_XIANGQI_FEN,
                    headersToXiangqi(headers),
                );
            } catch {
                xiangqiState = createXiangqiStateFromFen(
                    INITIAL_XIANGQI_FEN,
        ...
        if (position) {
            xiangqiState.path = position;
        }
        persistXiangqiState(id, xiangqiState);
    }
...
function headersToXiangqi(headers?: GameHeaders): XiangqiHeaderInput | undefined {
