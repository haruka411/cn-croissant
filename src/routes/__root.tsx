...
function RootLayout() {
  ...
  const showNotImplemented = useCallback(
    (feature: string) => {
      notifications.show({
        ...
        message: t("Common.FeatureNotImplemented", { feature }),
      });
    },
    [t],
  );
  ...
  const checkForUpdates = useCallback(() => {
    showNotImplemented(t("Menu.Help.CheckUpdate"));
  ...
  const menuActions: MenuGroup[] = useMemo(
        ...
        options: [
          {
            ...
            id: "documentation",
            action: () => showNotImplemented(t("Menu.Help.Documentation")),
          },
          {
            label: t("Menu.Help.ClearSavedData"),
            id: "clear_saved_data",
            action: () => {
              ask(t("Menu.Help.ClearSavedDataConfirm"), {
                title: t("Menu.Help.ClearSavedData"),
              }).then((res) => {
                if (res) {
                  localStorage.clear();
          ...
          },
          {
            ...
            id: "logs",
            action: async () => {
              const path = await resolve(await appLogDir(), "cn-croissant.log");
              notifications.show({
                title: t("Menu.Help.OpenLogs"),
                message: t("Menu.Help.LogsOpened", { path }),
              });
              await openPath(path);
            },
          },
    ...
    [t, checkForUpdates, createNewTab, keyMap, openNewFile, showNotImplemented, toggleFullscreen],
  );
  ...
  useEffect(() => {
    const unlisten = getCurrentWindow().listen(TauriEvent.DRAG_DROP, (event) => {
      ...
      if (payload?.paths) {
        const pgnFiles = payload.paths.filter((path) => /\.(pgn|xqf|txt)$/i.test(path));
