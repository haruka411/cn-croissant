  ...
  const onDragStop = () => {
    ...
    const handleDrop = async () => {
      ...
      try {
        ...
        if (node.type !== "directory" && NOTATION_FILE_REGEX.test(sourcePath)) {
          await rename(
            sourcePath.replace(NOTATION_FILE_REGEX, ".info"),
            targetPath.replace(NOTATION_FILE_REGEX, ".info"),
          ).catch(() => {});
        }
        ...
        if (selectedFile) {
          if (selectedFile.path === sourcePath) {
            const newName = sourceBasename.replace(NOTATION_FILE_REGEX, "");
