  ...
  const filteredEngines = useMemo(() => {
    ...
    return indexedEngines.filter(({ item }) => {
      const values = [
        ...
        item.type === "local" ? (item.protocol ?? "uci") : "",
...
function EngineSettings({
  ...
  const { data: options } = useSWRImmutable(
    ["engine-config", engine.path, engine.protocol ?? "uci"],
    async ([, path, protocol]) => {
      return unwrap(await commands.getEngineConfig(path, protocol));
    },
  );
        ...
        <Group grow align="start" wrap="nowrap">
          <Stack>
            ...
            <Select
              ...
              data={[
                { value: "uci", label: "UCI" },
                { value: "ucci", label: "UCCI" },
              ]}
              value={engine.protocol ?? "uci"}
              onChange={(value) =>
                setEngine({ ...engine, protocol: value === "ucci" ? "ucci" : "uci" })
              }
            />
          </Stack>
          <Center>
...
function EngineName({ engine }: { engine: LocalEngine }) {
