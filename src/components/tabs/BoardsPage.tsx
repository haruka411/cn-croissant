...
import { readPersistedXiangqiState } from "@/xiangqi/persistence";
import { XiangqiStateProvider } from "@/xiangqi/store";
...
import XiangqiPendingPage from "../xiangqi/XiangqiPendingPage";
...
export default function BoardsPage() {
  ...
  const closeTab = useCallback(
    async (value: string | null, forced?: boolean) => {
      if (value !== null) {
        ...
        const xiangqiState = readPersistedXiangqiState(value);
        const dirty = Boolean(tabState?.state?.dirty || xiangqiState?.dirty);
        if (isPersistentGameOrigin(closedTab) && dirty && !forced) {
          toggleSaveModal();
          return;
        }
        if (value === activeTab) {
  ...
  const duplicateTab = useCallback(
    (value: string) => {
      ...
      if (sessionStorage.getItem(`${value}-xiangqi`)) {
        sessionStorage.setItem(`${id}-xiangqi`, sessionStorage.getItem(`${value}-xiangqi`) || "");
      }
  ...
  return match(tab.type)
    ...
    .with("play", () => (
      <TreeStateProvider id={tab.value}>
        <XiangqiStateProvider id={tab.value}>
          <Mosaic<ViewId>
