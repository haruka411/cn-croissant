...
async getEngineConfig(path: string, protocol?: EngineProtocol) : Promise<Result<EngineConfig, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_engine_config", { path, protocol }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type GameConfig = { white: PlayerConfig; black: PlayerConfig; whiteTimeControl: TimeControl | null; blackTimeControl: TimeControl | null; initialFen: string | null; initialMoves?: string[]; openingBook: OpeningBookConfig | null; xiangqiRule?: XiangqiRepetitionRule }
export type GameEndReason = "checkmate" | "noLegalMove" | "perpetualCheck" | "perpetualChase" | "naturalDraw" | "repetition" | "timeout" | "resignation" | "abandonment"
...
export type XiangqiRepetitionRule = "AsianRule" | "ChineseRule" | "SkyRule" | "ComputerRule" | "YitianRule" | "AllowChase" | "NoJudgement"
...
export type OpeningBookConfig = { path: string; maxPly?: number }
...
const FALLBACK_MISSING = Symbol("fallback-missing");

async function TAURI_INVOKE<T = unknown>(command: string, args?: Record<string, unknown>): Promise<T> {
	try {
		return await RAW_TAURI_INVOKE<T>(command, args);
	} catch (error) {
		if (isTauriRuntime()) throw error;
		const fallback = fallbackInvoke(command, args);
		if (fallback !== FALLBACK_MISSING) return fallback as T;
		throw error;
	}
}
...
function fallbackInvoke(command: string, args?: Record<string, any>) {
	switch (command) {
		case "get_best_moves":
			return null;
		case "analyze_game":
		...
		case "merge_players":
		case "delete_game":
		case "delete_duplicated_games":
		case "delete_empty_games":
		case "clear_games":
		case "abort_game":
		case "resign_game":
			return null;
		...
		case "get_engine_config":
			return {
				name: fallbackName(args?.path, "Engine"),
				options: [],
			};
		case "file_exists":
			return true;
		case "get_file_metadata":
			return { last_modified: Date.now() };
		...
		case "get_db_info":
			return {
				title: fallbackName(args?.file, "Database"),
				description: "",
				player_count: 0,
				event_count: 0,
				game_count: 0,
				storage_size: BigInt(0),
				filename: fallbackName(args?.file, ""),
				indexed: false,
			};
		case "search_position":
			return [[], []];
		case "get_puzzle_db_info":
			return {
				title: fallbackName(args?.file, "Puzzle database"),
				description: "",
				puzzleCount: 0,
		...
		case "get_game_state":
		case "make_game_move":
		case "take_back_game_move":
			return fallbackGameState(args);
		default:
			return FALLBACK_MISSING;
	}
}

function fallbackName(value: unknown, fallback: string) {
	if (typeof value !== "string" || value.length === 0) return fallback;
	return value.split(/[\\/]/).filter(Boolean).at(-1) ?? fallback;
}

function fallbackGameState(args?: Record<string, any>) {
	const config = args?.config ?? {};
	const initialFen =
		config.initialFen ??
		"rnba1abnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1";
	return {
		gameId: args?.gameId ?? "fallback-game",
		status: "playing",
		initialFen,
		moves: [],
		...
		whiteTime: null,
		blackTime: null,
		whitePlayer: config.white?.name ?? "Red",
		blackPlayer: config.black?.name ?? "Black",
	};
}
